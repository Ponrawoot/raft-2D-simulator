...
public class Mission {
	...
	public Mission() {
		...
		allMission.get(0).addAll(Arrays.asList(0)); // wood -> stone, rope
		allMission.get(1).addAll(Arrays.asList(0, 0)); // scrape, rope -> metal
		allMission.get(2).addAll(Arrays.asList(0)); // fish -> stone spear
		allMission.get(3).addAll(Arrays.asList(0)); // bird -> metal spear
		allMission.get(4).addAll(Arrays.asList(0)); // fruit -> stone axe, +1 max HP
		allMission.get(5).addAll(Arrays.asList(0, 0, 0, 0)); // fruit, wood, leaf, metal -> metal axe
		allMission.get(6).addAll(Arrays.asList(0, 0)); // feather, bird -> stone shovel, +1 max HP
		allMission.get(7).addAll(Arrays.asList(0)); // metal -> titanium
		allMission.get(8).addAll(Arrays.asList(0, 0)); // plastic, stone -> copper
	}
...
//				Player.setStone(Player.getStone() + );
//				Player.setRope(Player.getRope() + );
			} else
				return false;
		case 1:
...
//				Player.setMetal(Player.getMetal() + );
			} else
				return false;
		case 2:
		...
		default:
			if (Player.getPlastic() >= selectedMission.get(0) && Player.getStone() >= selectedMission.get(1)) {
				...
				Player.setCopper(Player.getCopper() + 1);
			} else
				return false;
		}
		this.addRequirement(i, 20); // amount can be change
		return true;
	}
